...
if __name__ == "__main__":
    ...
    # Custom code: Lora
    # Note: ref_model cannot be wrapped with peft
    # 1. Define the LoRA Configuration
    config = LoraConfig(
        r=16, # Rank: higher = more parameters, lower = less VRAM
        lora_alpha=32,
        target_modules=["q_proj", "v_proj"], # Specific to model (e.g., Llama/Qwen)
        lora_dropout=0.05,
        bias="none",
        task_type=TaskType.CAUSAL_LM # Adjust based on your paper's task
    )
    ...
    # Custom code: Merge LoRA
    if trainer.is_world_process_zero():
        print("Fine-tuning finished. Merging and saving models...")
        # 1. Save the lora weights (Safe because it's a trainer method)
        trainer.save_model(training_args.output_dir + "_lora")
        
        # 2. Merge and Save full weights (Now safe because it's inside the 'if' block)
